...
let span_key = Eio.Fiber.create_key ()
       ...
       ( (),
         {
           Trace_core.Ambient_span_provider.Callbacks.get_current_span =
             (fun () -> Eio.Fiber.get span_key);
           with_current_span_set_to =
             (fun () span f ->
               Eio.Fiber.with_binding span_key span (fun () -> f span));
         } ))
...
let sink =
  Awskit_eio.Observability.Trace_sink.create ~name:"trace" ~needs_clock:true
    ...
    ~start:(fun started ->
      ...
      in
      {
        Awskit_eio.Observability.Trace_sink.within =
          (fun f -> Eio.Fiber.with_binding span_key span f);
        correlation = [];
        finish =
          (fun completion ->
            Exn.protect
