...
module Make (C : Awskit_s3_operation_context.S) = struct
  ...
  module Bucket_xml = Awskit_s3_bucket_config_xml
  ...
  let create conn ~bucket ?options () =
    ...
    | Ok () -> (
        ...
        let body =
          ...
          then ""
          else Awskit_s3_bucket_result_xml.create_config region
        in
        let upload = R.string_body body in
        let headers =
        ...
        | Ok request -> (
            ...
            | Ok (response, body) ->
                if Awskit.Response.is_success response then
                  return_ok { Bucket.Create.request = response }
                else error_response response body))
  ...
  let delete conn ~bucket =
    ...
    | Ok () -> (
        ...
        | Ok request -> (
            ...
            | Ok (response, body) ->
                if Awskit.Response.is_success response then
                  return_ok { Bucket.Delete.request = response }
                else error_response response body))
    ...
    | Ok request -> (
        ...
        | Ok (response, body) -> (
            if not (Awskit.Response.is_success response) then
              ...
              match body with
              | Error error -> return_error error
              | Ok body -> return (Awskit_s3_bucket_result_xml.parse_list body))
        )
        ...
        | Ok request -> (
            ...
            | Ok (response, body) -> (
                if not (Awskit.Response.is_success response) then
                  ...
                  | Error error -> return_error error
                  | Ok body ->
                      return (Awskit_s3_bucket_result_xml.parse_location body)))
        )
