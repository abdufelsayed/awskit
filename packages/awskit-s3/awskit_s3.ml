...
module Artifact_operation = Artifact_operation
module S3_observability = Observability
...
struct
  ...
  module Runtime = R
  include Execution.Kernel.Make (R) (Observer)
end

module Hard_off (R : RUNTIME) = struct
  type +'a io = 'a R.t
  type connection = R.connection
  type lease = unit

  let with_operation _ ~operation:_ ~start:_ callback = callback ()
  let emit_event _ _ ~data:_ = ()
  let acquire _ _ ~labels:_ _ = ()
  let add () _ = ()
  let release () = ()
  let with_instrument _ _ ~labels:_ _ callback = callback ()
end

module Make (R : RUNTIME) = Make_with_observer (R) (Hard_off (R))
