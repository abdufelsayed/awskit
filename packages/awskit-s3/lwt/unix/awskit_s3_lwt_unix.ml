  ...
  module Transfer = struct
    ...
    let upload_from_path conn ~bucket ~key ?options ?on_progress ~path () =
      Lwt.bind (upload_body_of_path ?on_progress path) (function
        | Error _ as error -> Lwt.return error
        | Ok body -> put conn ~bucket ~key ?options ~body ())
    ...
    let upload_missing_parts conn ~bucket ~key ~upload_id ~options ~path
      ...
      let upload_one spec =
        Lwt.bind
          (upload_part_from_path conn ~bucket ~key ~upload_id ~options ~path
             spec) (function
          | Error _ as error -> Lwt.return error
          | Ok part ->
    ...
    let complete_multipart conn ~bucket ~key ~upload_id upload parts =
      ...
      Lwt.bind (S3.Multipart.complete conn ~bucket ~key ~upload_id parts)
        ...
        | Ok complete ->
            Lwt.return_ok
              { Awskit_s3.Multipart.Managed.upload; parts; complete })

    let resume_multipart_upload_from_path conn ~bucket ~key ~upload_id ?options
        ?(concurrency = 4) ?on_progress ~path () =
      let options =
        Option.value ~default:Awskit_s3.Multipart.Managed.default_options
          options
      in
      ...
      | Ok () -> (
          ...
          | Ok () ->
              Lwt.bind (regular_file_length path) (function
                ...
                | Ok content_length -> (
                    match
                      multipart_specs ~content_length
                        ...
                        Lwt.bind
                          ...
                          | Ok created ->
                              ...
                              let abort_and_return error =
                                Lwt.bind
                                  (S3.Multipart.abort conn ~bucket ~key
                                     ~upload_id) (fun _ ->
    ...
    let download_to_path conn ~bucket ~key ?options ?on_progress ~path () =
      ...
      in
      Lwt.bind (get conn ~bucket ~key ?options ~consume ()) (function
        | Error _ as error -> Lwt.return error
