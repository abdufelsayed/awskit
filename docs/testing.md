...
| --- | --- | --- | --- |
| `@check-quick` | Fast no-service correctness for active development and CI. | Core contracts, recording-runtime support contracts, runtime HTTP workloads, simulator workloads, transfer fault workloads, protocol/domain properties, protocol fixtures, and protocol replay. | No |
| `@check-integration` | Service-backed S3 contract evidence. | MinIO S3 workload and bounded transfer cases. | Local MinIO |
...
the MinIO configuration explicitly so the alias cannot degrade into a
configuration skip:
