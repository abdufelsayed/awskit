...
| `@check-local` | Main no-network correctness gate for repository changes and CI. | Runtime HTTP workloads, simulator workloads, transfer fault workloads, protocol wire properties, protocol fixtures, and protocol replay. | No |
...
| S3 simulator state | `@s3-sim-workload` | S3 simulator | Model workload |
| S3 transfer faults | `@s3-transfer-faults` | S3 simulator | Fault workload |
| S3 protocol wire | `@s3-protocol-wire` | Pure protocol surface | Property workload |
