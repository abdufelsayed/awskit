...
| Simulator contracts | No-network S3 behavior, stateful model-oracle workloads, fault injection, and docs/test backend behavior. The simulator is not an AWS wire authority. | `opam exec -- dune build @s3-sim` |
...
| S3 simulator state | `@s3-sim` | S3 simulator | Model workload and deterministic simulator contracts |
...
`stress` sets `AWSKIT_QCHECK_COUNT` to `AWSKIT_STRESS_QCHECK_COUNT` when
`AWSKIT_QCHECK_COUNT` is unset. The default stress count is `2000`. Override it
explicitly for longer pressure runs:
