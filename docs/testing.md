...
| MinIO workload | Local adapter interoperability for the shared S3 state workload through a service-backed S3-compatible test double. Requires a local MinIO service and cleanup, and remains outside no-network gates. | `opam exec -- dune build --force @s3-minio-workload` |
