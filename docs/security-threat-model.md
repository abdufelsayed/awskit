...
| Credential chains continue after unavailable providers and stop on invalid configured credentials, provider failures, or expired credentials. Environment/profile and metadata providers preserve endpoint, cache, timeout, fallback, redaction, and native-cancellation policy. | Core/runtime tests plus `test/awskit/unix/test_credentials_unix.ml`, `test/awskit/lwt/test_credentials_lwt.ml`, and `test/awskit/lwt/unix/test_credentials_lwt_unix.ml` through `@credential-contracts`. | A provider family, refresh path, metadata endpoint, or runtime credential policy changes. |
