...
| Observability keeps bounded metric dimensions separate from diagnostics, removes sensitive diagnostics from Logs, Trace, and OpenTelemetry, and contains projection failures. | `test/observability/**` through `@observability-contracts`. | Observation dimensions, diagnostics, projections, or reporter bridges change. |
